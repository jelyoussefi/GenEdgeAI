...
RUN apt update -y && \
    ...
    python3-opencv libopencv-dev v4l-utils

RUN pip3 install Flask flask_bootstrap nncf fire psutil openvino-dev[onnx] zeroconf --break-system-packages
...
RUN pip3 install openvino-genai  --break-system-packages
RUN pip3 install optimum-intel@git+https://github.com/huggingface/optimum-intel.git --break-system-packages
